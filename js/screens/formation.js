...
function render(el) {
  ...
  const slotHTML = s.party.map((slot, i) => {
    const content = slotContent(slot, s);
    const active = _selectedSlot === i ? 'selected' : '';
    return `<div class="form-slot ${active}" data-idx="${i}">${content}</div>`;
  }).join('');
  ...
  const eggsHTML = s.eggs.map(inst => {
    ...
    return `<div class="box-item egg-item ${inParty?'in-party':''}" data-type="egg" data-iid="${inst.instanceId}" style="border-color:${def.color}">
      <div class="box-icon" style="background:${def.color}">ᾕa</div>
      <div class="box-info"><div class="box-name">${def.name}</div><div class="box-lv">Lv${inst.level} <small>(孵化用)</small></div></div>
    </div>`;
  }).join('');
  ...
  el.innerHTML = `
    <div class="form-wrap">
      ...
      <section class="form-party">
        ...
        ${_selectedSlot !== null ? '<div class="form-hint">下からモンスター/卵をタップしてセット ・ スロットを再度タップで解除</div>' : '<div class="form-hint">スロットをタップしてモンスター/卵を選ばう</div>'}
      </section>
      ...
      ${quest ? `<div class="form-footer"><button class="btn btn-primary btn-lg" id="btn-battle">⚔ 出撃！</button></div>` : ''}
    </div>
  `;

  el.querySelector('#btn-back').addEventListener('click', () => {
    navigateTo(_questId ? 'quest' : 'home');
  });
  ...
  if (btnBattle) {
    btnBattle.addEventListener('click', () => {
      const s2 = getState();
      const hasMonster = s2.party.some(p => p.type === 'monster');
      if (!hasMonster) {
...
function slotContent(slot, s) {
  ...
  if (slot.type === 'monster') {
    ...
    return `<div class="slot-filled" style="background:${def.color}22;border-color:${def.color}">
  ...
  if (slot.type === 'egg') {
    ...
    if (!def) return `<div class="slot-empty">✕</div>`;
    return `<div class="slot-filled egg-slot-filled" style="background:${def.color}22;border-color:${def.color}">
      <div class="slot-icon-big" style="background:${def.color}">ᾕa</div>
